<?php
/**
 * Sélection de salon — liste les salons récupérés depuis l'API et permet
 * de les rejoindre. Chaque carte pointe vers chat/index.php?channel=...
 */
...
<?php else: ?>
    ...
    <?php foreach ($channels as $chan):
        ...
        $name  = $chan['channel'];                       // ex: #accueil
        $label = ltrim($name, '#');                       // ex: accueil
        $users = isset($chan['users']) ? (int)$chan['users'] : 0;
        $href  = 'chat/index.php?channel=' . rawurlencode($label);
    ?>
